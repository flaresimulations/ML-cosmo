...
for output_name,marker,c in zip(
    ...
    # for ax,prop in zip(axes,     
    for ax,label,prop in zip(axes, ['$M_{\star}$', '$M_{\mathrm{gas}}$','$M_{\\bullet}$', 
                               '$v_{\mathrm{disp},\star}$','$\mathrm{SFR}$', '$Z_{\star}$'],
                              ['Stars_Mass_EA','MassType_Gas_EA','BlackHoleMass_EA',
        ...
        # ax.text(0.9,0.05,label, transform=ax.transAxes, horizontalalignment='right', fontsize=14)#, rasterized=False)
        ax.set_title(label, fontsize=14)
        ax.set_ylim(0.5,1) 
        ax.grid(b=True, which='both', alpha=0.5)
...
ax1.legend(handles=[L100_patch,L050_patch, L050_zoom_patch, line_, line_dens],# line_denszoom])#,
           frameon=False, loc='lower center', prop={'size': 13});
